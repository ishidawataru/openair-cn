...
START_TEST(imsi_convert_to_uint_test)
{
    ...
    for(i = 0; i < TEST_CASE_COMMON_CONVERT_MAX; i++){
        ...
        ck_assert_msg(imsi_uint64 == imsi_cmp[i], "Assertion 'imsi_uint64 == imsi_cmp[%i]' failed: imsi_uint64 == %ju, imsi_cmp[%i] == %ju",
          i, (uintmax_t) imsi_uint64, i, (uintmax_t) imsi_cmp[i]);
    }
